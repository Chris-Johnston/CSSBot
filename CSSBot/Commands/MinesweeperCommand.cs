...
namespace CSSBot
{
    public class MinesweeperCommand : RetryModuleBase
    {
        ...
        private MinesweeperSolutionService solutions;

        public MinesweeperCommand(MessageRetryService retryService, MinesweeperSolutionService solutions) : base(retryService)
        {
            random = new Random();
        ...
        public async Task Minesweeper(int x, int y, int numbombs)
        {
            ...
            if (msg != null)
            {
                ...
                if (solutions.LastPuzzleInChannel.ContainsKey(channelid))
                {
                    solutions.LastPuzzleInChannel[channelid] = msg.Id;
                }
                else
                {
                    solutions.LastPuzzleInChannel.Add(channelid, msg.Id);
                }
            }
        }
        ...
        public async Task Solve(IMessage message = null)
        {
            ...
            if (message == null)
            {
                ...
                if (solutions.LastPuzzleInChannel.ContainsKey(channelId))
                {
                    messageId = solutions.LastPuzzleInChannel[channelId];
                }
                else
                {
